<template>
  <div>
<!--    <button @click="click1()">点击测试</button>-->
<!--    <router-link to="/show">跳转到图表页面</router-link>-->
    <div class="top"></div>
    <div class="search">
      <el-form
        ref="form"
      ...
      >
        ...
        <el-form-item label="产品" prop="orderPerson">
          <el-input
...
export default {
  ...
  methods: {
    async tableList() {
      await axios
        .get('http://localhost:8080/api/QueryProductList')
        .then((res) => {
    ...
    Jump(data) {
      ...
      // this.$router.push({ name: '/Home2', params: { id: row.ProductKey } })
      // eslint-disable-next-line no-unused-vars
      const path = '/Home2/' + data.productKey
      this.$router.push({ path: path, query: { id: data.productKey } })
    },
    search() {
    ...
    save(data) {
      console.log(data)
      axios
        .post('http://localhost:8080/api/inputproduct', {
          productName: this.form.productName
          // ProductName: this.form.productName
        }).then((res) => {
          if (res.data) {
